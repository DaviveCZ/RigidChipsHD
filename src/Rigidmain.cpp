...
//Conversion efficiency from fuel (higher is better)
double ARM_EFF=1.0;
double JET_EFF=10.0;
double WHL_EFF=30.0;
...
GVector AirSpeed;//Wind (Aerodynamics?)
...
CD3DMesh*	m_pSkyMesh;				// XMesh data
CD3DMesh*	m_pXMesh[GMODELMAX];	// XMesh data
CD3DMesh*	m_pLandMesh;	// XMesh data
...
GVector EyePos2; //Viewpoint including KL<> movement
GVector UserEyePos;
GVector UserRefPos;
GVector UserUpVec;
int ViewUpdate=0;
...
DIACTION g_rgGameAction[] =
{
    ...
    { INPUT_INIT,	DIKEYBOARD_HOME,	0, TEXT("Init"), },
    { INPUT_RESET,	DIKEYBOARD_R,	0, TEXT("Reset"), },
    { INPUT_TITLE,	DIKEYBOARD_INSERT,	0, TEXT("Show About"), },
    { INPUT_ZOOMIN,	DIKEYBOARD_NUMPAD7,	0, TEXT("Zoom-in"), },
    { INPUT_ZOOMOUT,DIKEYBOARD_NUMPAD9,	0, TEXT("Zoom-out"), },
    { INPUT_RESETVIEW,	DIKEYBOARD_NUMPAD0,	0, TEXT("Reset View"), },
	{ INPUT_TUP,	DIKEYBOARD_NUMPAD8,	0, TEXT("Turn Up"), },
	{ INPUT_TDOWN,	DIKEYBOARD_NUMPAD5,	0, TEXT("Turn Down"), },
	{ INPUT_TLEFT,	DIKEYBOARD_NUMPAD4,	0, TEXT("Turn Left"), },
	{ INPUT_TRIGHT,	DIKEYBOARD_NUMPAD6,	0, TEXT("Turn Right"), },
...
void AttackDataDisp(char *s,DPNID dpnid,int attack){
	...
	if(frameGetTime_old!=frameGetTime){ //Limit the update frequency to 1F1 times. In the first place, drawing should not be updated here, but there are cases where the reflection of simultaneous impacts is delayed.
		...
		//SendMessage(GetDlgItem(NetworkDlg,IDC_ATTACKTEXT),EM_SETSEL,0,-1); //select all
		//SendMessage(GetDlgItem(NetworkDlg,IDC_ATTACKTEXT),EM_SETSEL,-1,-1); //lift
		//SendMessage(GetDlgItem(NetworkDlg,IDC_ATTACKTEXT),EM_REPLACESEL,0,(WPARAM)"_");
		SendMessage(GetDlgItem(NetworkDlg,IDC_ATTACKTEXT),EM_LINESCROLL,0,GMAXATTACKLINE+1); //Scroll to the cursor position ･･･ Scroll the maximum number of lines because it could not be done
	}
}
...
char  *ChatDataDisp(char *name,char *s){
	...
	SendMessage(GetDlgItem(NetworkDlg,IDC_CHATTEXT),EM_SETSEL,0,-1); //select all
	SendMessage(GetDlgItem(NetworkDlg,IDC_CHATTEXT),EM_SETSEL,-1,-1); //lift
...
HRESULT MyReceiveFunc( MYAPP_PLAYER_INFO* playerInfo,DWORD size,BYTE *stream ) {
	...
	memcpy(strm,stream,size); //copy once
	...
	if(code==-1) {
		...
		for(int i=0;i<DPlay->GetMaxPlayers();i++) {
			if(PlayerData[i].ReceiveData.info.dpnidPlayer==playerInfo->dpnidPlayer) {
				...
				PlayerData[i].sendtime=PlayerData[i].sendtime2=PlayerData[i].ReceiveData.data[0].color; //For now, only (C8) core detailed coordinates should be enough
				...
				PlayerData[i].rtime2=PlayerData[i].rtime2+PlayerData[i].span2+5; //For the time being, let's add one
				DWORD timeGt=frameGetTime;
				if(timeGt-PlayerData[i].rtime>300 || timeGt-PlayerData[i].rtime2>300){ //Reset when the accumulated error of the partner's internal time calculated locally exceeds 60 or is less than 0
	...
	else if(code==30) { //15B26 added Scenario message
		for(int i=0;i<DPlay->GetMaxPlayers();i++) {
			if(PlayerData[i].ReceiveData.info.dpnidPlayer==playerInfo->dpnidPlayer) {
	...
	else if(code==10) { //15B20 added Send only Core location information
		for(int i=0;i<DPlay->GetMaxPlayers();i++) {
			if(PlayerData[i].ReceiveData.info.dpnidPlayer==playerInfo->dpnidPlayer) {
				...
				PrePlayerData[i].sendtime2=PlayerData[i].sendtime2;
				PlayerData[i].sendtime2=PlayerData[i].ReceiveData.data[0].color; //For now, only (C8) core detailed coordinates should be enough
				...
				DWORD timeGt=frameGetTime;
				if(timeGt-PlayerData[i].rtime2>300){ //Reset when the accumulated error of the partner's internal time calculated locally exceeds 60 or is less than 0
...
INT_PTR CALLBACK DlgDataProc(HWND hDlg,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	char str[1000];
	char *s;
	FILE *fp;
...
INT_PTR CALLBACK DlgExtraProc(HWND hDlg,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	int p;
	static int state=0;
	char str[1000];
...
INT_PTR CALLBACK DlgNetworkProc(HWND hDlg,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	static int state=0;
	...
	if(mesh) {
		...
		if(MeshNo>=0) { //地形以外なら色を変える
			...
			if(MeshNo==10 && (Option==1||Option==2)) {
				//uncomment these to stop scaling of balloons
...
void GRigid::DispJet()
{
	...
	if(JetFlag && MeshNo==10 && PowerByFuel!=0 && Top!=NULL &&  Option==0 ) {
		...
		D3DXMatrixMultiply( &matLocal , &mat1, &matLocal);
		//Jet effect scale
		D3DXMatrixScaling( &mat1, (FLOAT)0.8,(FLOAT)f,(FLOAT)0.8 );
		D3DXMatrixMultiply( &matLocal , &mat1, &matLocal);
		D3DXMatrixTranslation(&mat1,0.0f,-0.32f,0.0f);
		D3DXMatrixMultiply( &matLocal , &mat1, &matLocal);
		D3DXMatrixMultiply( &matLocal , &matLocal, &GMatWorld);
...
//	D3DXCreateTextureFromFile(m_pd3dDevice,"dustw.png",&pPointTexture);
    ...
    // Set up lighting states (Sun light)
    D3DUtil_InitLight( light, D3DLIGHT_DIRECTIONAL, 0.0f, -1.0f, -0.5f );
	light.Specular=D3DXCOLOR(1.0f,1.0f,1.0f,0.3f);
...
*/
	...
	bool mouseWithinWindow = false;

	GetCursorPos(&pos);
	if (pos.x >= rect.left && pos.x <= rect.right && pos.y >= rect.top && pos.y <= rect.bottom && GetFocus())
		mouseWithinWindow = true;
	else
		mouseWithinWindow = false;

	ScreenToClient(m_hWnd,&pos);
	MouseDX = pos.x - MouseX;
...
HRESULT CMyD3DApplication::ViewSet() {
	...
	int c=0;
	//Camera Distance
	GFloat r=CameraDistance+Chip[0]->V.abs()/40.0f;
	...
	{
		UpVec = GVector(0, 1, 0);
		EyePos = Chip[LastBye]->TotalCenter + GVector(0, r * 0.1f, r);
		EyePos.y = Chip[LastBye]->TotalCenter.y + 2.0f;
		RefPos = Chip[LastBye]->TotalCenter + GVector(0, 0, 0);
		UserRefPos = RefPos; UserEyePos = EyePos; UserUpVec = UpVec;
...
//		World->Disp(DPlay->GetNumPlayers()!=0);
		...
		//	m_pd3dDevice->SetTextureStageState(0, D3DTSS_MIPFILTER, D3DTEXF_LINEAR);
			...
			if(s<=0) s=64;
			//this changes water scroll speed for the texture...
			GFloat si=(GFloat)sin(count/250.0*M_PI)-30;
			...
			case IDM_CHANGEVIEW1:
				{
					if(ViewType==0) {
                        int i;
						for(i=LastBye+1;i<World->ChipCount;i++) {
